    ...
    withContext(Dispatchers.IO) {
      ...
      if (cleanKey.isBlank()) {
        throw IllegalArgumentException(
          "Claude API Key is blank. Please enter your API key in Profile settings."
        )
      }
      ...
      val payload =
        JsonObject().apply {
          ...
          add(
            "messages",
            gson.toJsonTree(
              listOf(mapOf("role" to "user", "content" to "Log this meal: $inputText"))
            )
          )
        }
