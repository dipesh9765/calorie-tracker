...
/**
 * Concrete adapter implementation for Google Generative AI REST API (`gemini-3.6-flash` / `gemini-3.5-flash`).
 *
 * Responsibilities:
...
class GeminiAdapter(
    ...
    override suspend fun parseMeal(inputText: String, apiKey: String, systemInstruction: String, modelName: String): String = withContext(Dispatchers.IO) {
        ...
        if (cleanKey.isBlank()) {
            throw IllegalArgumentException("Gemini API Key is blank. Please enter your API key in Profile settings.")
        }
        ...
        try {
            ...
            if (message.contains("404") || message.contains("not found") || message.contains("NOT_FOUND")) {
