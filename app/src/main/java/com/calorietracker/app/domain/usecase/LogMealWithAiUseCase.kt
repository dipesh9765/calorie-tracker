...
class LogMealWithAiUseCase(
     ...
     *
     * @param inputText Natural language text entered by the user (e.g., "2 rotis with chicken curry").
     * @param provider Configured AI provider enum ([AiProviderType.GEMINI], etc.).
     * @param apiKey User's API key for the chosen provider.
     * @return [Result] containing a [Pair] of the saved [Meal] domain object and AI coach advice string.
     */
    suspend fun execute(
        inputText: String,
        ...
        return result.onSuccess { (meal, _) ->
