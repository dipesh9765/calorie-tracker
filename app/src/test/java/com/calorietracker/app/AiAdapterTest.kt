...
/**
 * Functionality tests for all AI Providers (Gemini, OpenAI, Claude, DeepSeek). Tests JSON
 * extraction, adapter response parsing, error handling, and payload schemas.
 */
class AiAdapterTest {
  ...
  fun `extractJsonString parses markdown wrapped JSON correctly`() {
    ...
    val expected =
      """{"foodName": "Tarri Poha","portionDescription": "1 plate","calories": 300}"""
        .replace(" ", "")
  ...
  fun `mock adapter parseMealText returns expected meal domain model`() = runBlocking {
    ...
    val result =
      testRepo.parseMealText("3 rotis with saoji chicken", AiProviderType.GEMINI, "test_api_key")
  ...
  fun `mock adapter parseMealText handles yesterday past date parsing`() = runBlocking {
    ...
    val result =
      testRepo.parseMealText(
        "Yesterday night I ate 2 rotis with egg curry",
